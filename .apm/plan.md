---
title: TeaMode
modified: Plan creation by the Planner. Manager renamed package teamode/ → app/ before Stage 1 dispatch. T4.2 guidance updated for new end-of-session embed + reflect-prompt copy + broadened @-mention design before T4.2 dispatch. T4.2 guidance rewritten: dropped Y/N/End-early buttons + WhyModal; facilitator's ✅/⛔ reaction now authoritative for `completed_intention`; ⛔ posts public "why" prompt (not bookkept) — before T4.2 follow-up dispatch.
---
...
* **Objective:** At zero, post the Session-complete embed (single message: `content` with @-mentions of voice members, `embed` with title/body), trigger reverie, post the Reflect embed with facilitator-prompt `content` and pre-populated ✅/⛔ reactions, accept facilitator reaction as authoritative state write, post public "why" prompt on ⛔ (no capture), 3-minute timeout watchdog for no-reaction.
...
* **Guidance:** At countdown zero, sequence per Spec § "Session Lifecycle — Sequence of bot actions" and the canonical copy blocks in `.project-meta/UI-ADR.md`: **(1) Session-complete embed** — single `channel.send(content=..., embed=...)` with `content="Time's up, <@id1> <@id2> ...!"` snapshotting voice members at end-tick (filter Ocha; empty → `"Time's up!"`) and embed using canonical title (`✨ Session complete!`) + body (`🌿 Sip your tea, stretch, and notice your progress.`) + `#3F5E4A` accent. Mentions in `content` ping; in embed they don't. **(2) Reverie + disconnect** — `voice.play_reverie_then_disconnect()` from T4.1; bool return informational. **(3) Reflect embed** — single `channel.send(content=..., embed=...)` with `content="Facilitator <@facilitator_id>! React ✅ if you finished, ⛔ if not."` and embed using the canonical Reflect copy (title `🌿 [Reflect]`, bullet body, `#3F5E4A` accent). After posting, bot calls `await message.add_reaction("✅")` and `add_reaction("⛔")`. **(4) Reaction listener handles state writes** — on reaction-add events for the Reflect embed message, filter (a) by emoji (only ✅ and ⛔ matter), (b) by user (skip the bot's own reactions). If the reactor is the **current facilitator** (`session.facilitator_id`), the reaction is **authoritative**: ✅ → `mark_completed(completed_intention=1, followup_note=None)` and watchdog cancelled; ⛔ → `mark_completed(completed_intention=0, followup_note=None)`, watchdog cancelled, post public "why" prompt verbatim from UI-ADR (`<@facilitator_id> — share what got in the way: type in chat or share in voice.`). The bot does **not** capture the response (no Message Content Intent, no chat listener — `followup_note` stays NULL). Other voice members' ✅/⛔ reactions are logged via console (`logger.info`) only. **(5) 3-min watchdog** — cancellable `asyncio.Task` that fires `mark_followup_timeout(session_id=...)` if no facilitator reaction within 180s. The Yes/No/End-early button row, `WhyModal`, and the `on_interaction` `followup:*` dispatch branches from the prior T4.2 implementation are **removed** in this redesign. **Also:** confirm the premature `voice_client.disconnect()` from T3.3 is gone (`play_reverie_then_disconnect` handles disconnect).
