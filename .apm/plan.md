---
title: TeaMode
modified: Plan creation by the Planner. Manager renamed package teamode/ → app/ before Stage 1 dispatch. T4.2 guidance updated for new end-of-session embed + reflect-prompt copy + broadened @-mention design before T4.2 dispatch.
---
...
* **Objective:** At zero, post the end-of-session embed, participant follow-up prompt, trigger reverie, @-mention everyone currently in voice (snapshot at end-tick), post the follow-up button row, accept reactions, handle Y/N answers (with optional "why"), 3-minute timeout, and "end early" button.
...
* **Guidance:** At countdown zero, sequence per Spec § "Session Lifecycle — Sequence of bot actions" and the canonical end-of-session copy in `.project-meta/UI-ADR.md`: (1) post end-of-session embed (steeping forest `#3F5E4A` accent — title `✨ Session complete!`, body `🌿 Sip your tea, stretch, and notice your progress.`); (2) post participant follow-up prompt verbatim from Spec § "Participant flow": `🌿 **[Reflect]** Share how your session went in voice or type it in the chat.`; (3) trigger `voice.play_reverie_then_disconnect()` from Task 4.1 — its bool return tells you whether reverie audio actually played; (4) post the end-of-session @-mention always: `Time's up, <@id1> <@id2> ...!` snapshotting the voice channel's current members at end-tick (filter out Ocha herself). If the voice channel is empty, post `Time's up!` plain. This @-mention triggers Discord notifications and doubles as the audio fallback when reverie failed; (5) post the follow-up button row with three buttons: `Yes`, `No`, `End early` (all facilitator-only). Reactions: the message accepts 👍/👎 reactions from anyone; a reaction handler logs them via console only — they do not affect state. Y click: `session.mark_completed(...)` with `completed_intention=True`. N click: open a follow-up `Modal` for the "why" text; on submit, `session.mark_completed(...)` with `completed_intention=False, followup_note=<text>`. End early click (facilitator only, ephemeral refusal otherwise): close the window without recording an answer — `session.mark_completed(...)` with `completed_intention=NULL`. 3-minute timeout watchdog: if no facilitator action in 3 minutes, `session.mark_followup_timeout(...)`. All terminal transitions write `ended_at` per Spec. **Also:** remove the premature `voice_client.disconnect()` added by T3.3 around `app/bot.py:182` — `play_reverie_then_disconnect()` handles disconnect after playback.
