# APM Auto 1.0.0 - Work Breakdown Guide
...
`CLAUDE.md` defines how work is performed - execution patterns that apply across all or most Tasks. Claude Code auto-loads `CLAUDE.md` into every subagent context. The APM Rules block is a namespace to separate APM-managed standards from pre-existing content.
...
**Self-containedness:** Subagents' working context is intentionally scoped to their Task Prompt and `CLAUDE.md` - the Spec, Plan, and external design artifacts are omitted by design. Standards referencing those documents undermine that scoping. Embed content directly.
